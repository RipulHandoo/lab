...
public class EightPuzzleSolverBFSWithCost {
    public static void main(String[] args) {
        ...
        EightPuzzleSolverBFSWithCost solver = new EightPuzzleSolverBFSWithCost();
    ...
    private List<Node> getNeighbors(Node node) {
        ...
        for (int[] direction : directions) {
            ...
            if (x >= 0 && x < node.state.length && y >= 0 && y < node.state[0].length) {
                ...
                Node neighbor = new Node(neighborState, node, node.cost + 1);
