...
public class EightPuzzleSolverBestFS {
    public static void main(String[] args) {
        ...
        EightPuzzleSolverBestFS solver = new EightPuzzleSolverBestFS();
    ...
    public List<int[][]> solvePuzzle(int[][] initial, int[][] goal) {
        ...
        Node root = new Node(initial, null, calculateHeuristic(initial, goal));
    ...
    private List<Node> getNeighbors(Node node, int[][] goal) {
        ...
        for (int[] direction : directions) {
            ...
            if (x >= 0 && x < node.state.length && y >= 0 && y < node.state[0].length) {
                ...
                int heuristic = calculateHeuristic(neighborState, goal);

                Node neighbor = new Node(neighborState, node, heuristic);
